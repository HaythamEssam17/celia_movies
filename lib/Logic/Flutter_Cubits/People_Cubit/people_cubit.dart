...
class PeopleCubit extends Cubit<PeopleStates> {
  ...
  List<PersonModel> localPersons = [];
  List<PersonModel> get getLocalPersons => localPersons;
  ...
  void fetchLocalData(BuildContext context) async {
    localPersons.clear();

    var result = await DefaultSharedPrefs.getPeoples();
    logPrint('persons from json decode:1 $result');

    var jDecode = json.decode(result);
    logPrint('persons from json decode:2 ${localPersons.length}');

    localPersons =
        (jDecode as List).map((e) => PersonModel.fromJson(e)).toList();

    logPrint('persons from json decode:3 ${localPersons.length}');

    emit(FetchPeoplesLocalState());
  }
}
